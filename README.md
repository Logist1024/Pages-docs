...
- **一键复制 Markdown 源码**：阅读页标题下方「复制 Markdown」按钮，复制内容与页面所见一致
  （登录预览草稿时复制草稿源码）；也可直接访问 `<文档路径>?format=md` 获取纯文本源码
- **站点设置**：后台可改站点名称、首页地址（访客直接访问根地址时的默认落地页，也是点击站名的跳转目标）、顶部导航栏
  （居右加粗，搜索框居左；**站外导航项自动展示对方站点图标**——同源代理抓取 favicon，右侧渲染外开箭头），
  还能上传**浏览器标签页图标（favicon）**与**界面 LOGO**、配置**页眉公告栏**与**全站页脚**
...
- **深色 / 浅色模式**：阅读站与管理后台均支持跟随系统 + 手动切换；
  ...
  SSR 结构升级时旧缓存自动失联
- **移动端适配**：窄屏下文档目录变为左侧抽屉（点左上按钮滑出、点遮罩关闭），
  顶部导航与搜索框折叠进站点菜单面板；纯 CSS checkbox-hack 开合，禁用 JS 也能用
- **用量看板**：管理后台内置 Cloudflare 免费额度可视化（D1 存储 / R2 容量 / KV 键数 /
  Workers 请求数参考值），含近 30 天版本发布柱状图与占用明细
- **目录与文档**：目录和文档都支持随时改名与改路径（移动目录会级联搬移其下全部内容）；
  **同级任意排序**——后台侧栏悬停行点「↑」「↓」即可调整，阅读站侧栏与后台共用同一顺序；
  `docs`、`admin`、`api` 等系统保留路径禁止创建（前后端双重校验）；
  后台侧栏宽度也可拖拽调节并记忆
...
Cloudflare Worker（唯一入口，Hono）
  ...
  │               （文档路径直接映射到站点根路径；旧 /docs/* 链接仅在无真实文档时 302 跳转，
  │                自建 docs 目录不受影响；/admin、/api、/f、/assets、/icon 等保留前缀不作为文档路径解析）
  ├─ /api/*       JSON API：登录、站点设置、文档 CRUD、版本、排序、上传、搜索（除登录与设置读取外全部鉴权）
  ├─ /search      公开搜索页（FTS5）
  ├─ /icon/:host  站外导航项的站点图标代理（服务端抓取 favicon，防 CSP 拦截且不泄露访客 IP）
